...
export function PeoplePage() {
    ...
    // Handlers for data loading
    const handlePersonsLoaded = (loadedPersons: any[]) => {
        console.log('Pessoas carregadas:', loadedPersons);
        // Here you would transform the data into userProfiles format
        ...
        <div className="px-4 sm:px-6 lg:px-8 py-8">
            {!selectedUser ? (
                <>
                    ...
                    {!isDataLoaded && (
                        <PersonLoader onPersonsLoaded={handlePersonsLoaded} />
                    )}

                    {/* Existing barra de pesquisa e filtros - only show when data is loaded */}
                    {isDataLoaded && (
                    ...
                    {/* Existing grid de cartões de pessoas - only show when data is loaded */}
                    {isDataLoaded && (
                        <>
                            ...
                            {/* Existing mensagem quando não há resultados */}
                ...
                <div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="lg:col-span-1">
                            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-6">
                                <div className="flex flex-col items-center text-center mb-6">
                                    <div className="h-24 w-24 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 flex items-center justify-center text-white text-2xl font-bold mb-4">
                                ...
                                <div className="space-y-3 text-sm">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className="text-gray-900 dark:text-white font-medium">{selectedUser.department}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className="text-gray-900 dark:text-white font-medium">{selectedUser.location}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-gray-500 dark:text-gray-400">Data de Contratação</span>
                                        <span className="text-gray-900 dark:text-white font-medium">{selectedUser.hireDate}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-gray-500 dark:text-gray-400">Gestor</span>
                                        <span className="text-gray-900 dark:text-white font-medium">{selectedUser.manager}</span>
                                    </div>
                                </div>
                            </div>

                            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                                <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-4">Competências</h3>
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                                <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-4">Sobre</h3>
                            ...
                            <PerformanceChart
                                ...
                                type="line"
                                data={selectedUser.evaluationHistory.map(eval => ({
                                    name: `${eval.year} ${eval.quarter}`,
                                    value: eval.score
                                }))}
                            />

                            {/* Radar de competências */}
                            {selectedUser.evaluationHistory.length > 0 && (
                                <ComparisonChart
                                    title="Avaliação por Competência"
                                    type="radar"
                                    data={selectedUser.evaluationHistory[0].indicators.map(indicator => ({
                            ...
                            {/* Distribuição de peso dos indicadores */}
                            {selectedUser.evaluationHistory.length > 0 && (
                                <ComparisonChart
                                    title="Peso dos Indicadores"
                                    type="pie"
                                    data={selectedUser.evaluationHistory[0].indicators.map(indicator => ({
                                        name: indicator.name,
                            ...
                            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                                ...
                                <div className="space-y-4">
                                    <div className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
                                        <h4 className="font-medium text-gray-900 dark:text-white">Áreas para Desenvolvimento</h4>
                                        <ul className="mt-2 list-disc list-inside text-gray-600 dark:text-gray-400">
                                            <li>Aprimorar habilidades de {selectedUser.evaluationHistory[0]?.indicators.sort((a, b) => a.score - b.score)[0]?.name || 'liderança'}</li>
                                            <li>Desenvolver conhecimentos em novas tecnologias</li>
                                            <li>Fortalecer colaboração entre equipes</li>
                                        </ul>
                                    </div>
                                    <div className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
                                        <h4 className="font-medium text-gray-900 dark:text-white">Próximas Etapas</h4>
                                        <ul className="mt-2 list-disc list-inside text-gray-600 dark:text-gray-400">
                                            <li>Participar do programa de mentoria</li>
                                            <li>Treinamento específico em gestão de projetos</li>
                                            <li>Feedback regular com gestor direto</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
