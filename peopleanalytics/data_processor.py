...
class DataProcessor:
    ...
    def __init__(
        ...
        self.output_path = (
            Path(output_path).resolve() if output_path else Path("output").resolve()
        )
    ...
    def _setup_logging(self):
        ...
        self.logger.info(
            f"DataProcessor initialized with data_path={self.data_path}, output_path={self.output_path}"
        )
    ...
    def _validate_schema(self, data: Dict) -> bool:
        ...
        try:
            ...
            if (
                "conceito_ciclo_filho_descricao" not in data["data"]
            ...
            for direcionador in data["data"]["direcionadores"]:
                ...
                if (
                    "direcionador" not in direcionador
                    or "pergunta_final" not in direcionador
                    or "comportamentos" not in direcionador
                ):
                    return False
                ...
                for comportamento in direcionador["comportamentos"]:
                    ...
                    if (
                        "comportamento" not in comportamento
                        or "pergunta_final" not in comportamento
                        or "avaliacoes_grupo" not in comportamento
                    ):
                        return False
                    ...
                    for avaliacao in comportamento["avaliacoes_grupo"]:
                        ...
                        if (
                            "avaliador" not in avaliacao
                            or "frequencia_colaborador" not in avaliacao
                            or "frequencia_grupo" not in avaliacao
                        ):
                            return False

                        # Check frequencia arrays
                        if not isinstance(
                            avaliacao["frequencia_colaborador"], list
                        ) or not isinstance(avaliacao["frequencia_grupo"], list):
                            return False
    ...
    def generate_report(self) -> str:
                    ...
                    for comportamento in direcionador["comportamentos"]:
                        for avaliacao in comportamento["avaliacoes_grupo"]:
                            row = {
                                ...
                                "frequencia_colaborador": sum(
                                    i * v
                                    for i, v in enumerate(
                                        avaliacao["frequencia_colaborador"]
                                    )
                                )
                                / sum(avaliacao["frequencia_colaborador"]),
                                "frequencia_grupo": sum(
                                    i * v
                                    for i, v in enumerate(avaliacao["frequencia_grupo"])
                                )
                                / sum(avaliacao["frequencia_grupo"]),
                            }
                            all_data.append(row)
        ...
        with pd.ExcelWriter(report_file) as writer:
            # Overall summary
            summary = (
                df.groupby(["pessoa", "ano", "cargo", "nivel", "conceito"])
                .agg({"frequencia_colaborador": "mean", "frequencia_grupo": "mean"})
                .reset_index()
            )
            ...
            for direcionador in df["direcionador"].unique():
                ...
                direcionador_df.to_excel(
                    writer, sheet_name=direcionador[:31], index=False
                )
    ...
    def generate_summary(self, format: str = "html") -> str:
        ...
        if format == "html":
            # Load template
            template_path = (
                Path(__file__).parent.parent / "assets" / "templates" / "summary.html"
            )
            ...
            for person in df["pessoa"].unique():
                ...
                for year in person_data["ano"].unique():
                    ...
                    content.append(
                        "<tr><th>Direcionador</th><th>Comportamento</th><th>Média</th></tr>"
                    )

                    for direcionador in year_data["direcionador"].unique():
                        direcionador_data = year_data[
                            year_data["direcionador"] == direcionador
                        ]
                        for comportamento in direcionador_data[
                            "comportamento"
                        ].unique():
                            comportamento_data = direcionador_data[
                                direcionador_data["comportamento"] == comportamento
                            ]
                            media = comportamento_data["frequencia_colaborador"].mean()
                            content.append(
                                f"<tr><td>{direcionador}</td><td>{comportamento}</td><td>{media:.2f}</td></tr>"
                            )
                ...
                for year in person_data["ano"].unique():
                    ...
                    for direcionador in year_data["direcionador"].unique():
                        ...
                        for comportamento in direcionador_data[
                            ...
                            media = comportamento_data["frequencia_colaborador"].mean()
                            content.append(
                                f"| {direcionador} | {comportamento} | {media:.2f} |"
                            )
    ...
    def generate_mermaid_chart(self) -> str:
        ...
        for person in df["pessoa"].unique():
            ...
            for direcionador in person_data["direcionador"].unique():
                direcionador_data = person_data[
                    person_data["direcionador"] == direcionador
                ]
                avg_score = direcionador_data["frequencia_colaborador"].mean()
                content.append(
                    f'    {direcionador.replace(" ", "_")}["{direcionador}: {avg_score:.2f}"]'
                )
                content.append(
                    f"    style {direcionador.replace(' ', '_')} fill:#bbf,stroke:#333,stroke-width:1px"
                )
            ...
            content.append("graph TD")
            content.append(
                f'    title["Individual vs. Group Performance for {person}"]'
            )
            content.append("    style title fill:#f9f,stroke:#333,stroke-width:4px")
            ...
            for direcionador in person_data["direcionador"].unique():
                ...
                content.append(
                    f'    {direcionador.replace(" ", "_")}_diff["{direcionador} Difference: {diff:.2f}"]'
                )
                if diff >= 0:
                    content.append(
                        f"    style {direcionador.replace(' ', '_')}_diff fill:#9f9,stroke:#333,stroke-width:1px"
                    )
                else:
                    content.append(
                        f"    style {direcionador.replace(' ', '_')}_diff fill:#f99,stroke:#333,stroke-width:1px"
                    )
            ...
            # Save mermaid file
            output_file = (
                output_dir / f"mermaid_{person.replace(' ', '_')}_{timestamp}.md"
            )
            with open(output_file, "w", encoding="utf-8") as f:
                f.write("\n".join(content))
    ...
    def generate_ai_prompt(self) -> str:
        ...
        for person, years_data in person_data.items():
            ...
            content.append("## TASK")
            content.append(
                "Generate a comprehensive performance feedback report based on the evaluation data provided below. The report must be:"
            )
            content.append("")
            content.append(
                "- **Personalized:** Reference the employee's specific name, role, and performance data throughout"
            )
            content.append(
                "- **Balanced:** Highlight both strengths and development areas with equal attention"
            )
            content.append(
                "- **Actionable:** Include specific, practical recommendations that can be implemented"
            )
            content.append(
                "- **Data-driven:** Reference specific scores and comparison points provided in the data"
            )
            content.append(
                "- **Growth-oriented:** Frame feedback as opportunities for development rather than criticisms"
            )
            content.append(
                "- **Professional:** Maintain a formal but supportive tone throughout"
            )
            content.append("")
            ...
            content.append("")
            content.append(
                "1. **Executive Summary** (2-3 paragraphs summarizing overall performance)"
            )
            content.append(
                "2. **Key Strengths** (3-5 bullet points with specific examples from the data)"
            )
            content.append(
                "3. **Development Opportunities** (3-5 bullet points with specific examples from the data)"
            )
            content.append(
                "4. **Competency Analysis** (One section for each competency area with:)"
            )
            ...
            content.append(
                "5. **Stakeholder Perspectives** (Analysis of how different evaluators perceive performance)"
            )
            content.append(
                "6. **Development Plan** (3-5 specific actions with timeframes and success measures)"
            )
            ...
            content.append("")
            content.append(
                '- "Based on your consistently high scores in [specific competency] (4.2/5.0, +0.8 above peers), you demonstrate exceptional ability to [specific behavior]. Continue leveraging this strength by..."'
            )
            content.append(
                '- "While you show competence in [specific area] (3.1/5.0), there\'s opportunity to further develop by focusing on [specific behavior]. Your manager particularly noted that..."'
            )
            content.append(
                '- "A concrete development step would be to [specific action] over the next quarter, with success measured by [specific outcome]."'
            )
            content.append("")
            ...
            content.append(
                f"- **Years of Evaluation Data:** {', '.join(y['year'] for y in years_data)}"
            )
            content.append(
                f"- **Most Recent Overall Assessment:** {latest_year['overall_assessment']}"
            )
            content.append("")
            ...
            for year_data in years_data:
                ...
                content.append(
                    f"- **Overall Assessment:** {year_data['overall_assessment']}"
                )
                content.append("")
                ...
                for comp in year_data["competencies"]:
                    ...
                    content.append(
                        f"- **Assessment Question:** {comp['final_question']}"
                    )
                    content.append("")
                    ...
                    for behavior in comp["behaviors"]:
                        ...
                        content.append(
                            f"- **Behavior Question:** {behavior['final_question']}"
                        )
                        content.append("")
                        ...
                        for eval in behavior["evaluations"]:
                            ...
                            content.append(
                                f"- **Individual Score:** {eval['individual_score']:.2f}/4.0"
                            )
                            content.append(
                                f"- **Group Score:** {eval['group_score']:.2f}/4.0"
                            )
                            content.append(
                                f"- **Score Difference:** {(eval['individual_score'] - eval['group_score']):.2f}"
                            )
                            ...
                            all_scores.append(
                                {
                                    ...
                                    "difference": eval["individual_score"]
                                    - eval["group_score"],
                                }
                            )
            ...
            if all_scores:
                ...
                for strength in top_strengths:
                    content.append(
                        f"- **{strength['competency']} - {strength['behavior']}:** {strength['score']:.2f}/4.0 (Evaluated by {strength['evaluator']})"
                    )
                content.append("")
                ...
                for area in development_areas:
                    content.append(
                        f"- **{area['competency']} - {area['behavior']}:** {area['score']:.2f}/4.0 (Evaluated by {area['evaluator']})"
                    )
                content.append("")
                ...
                for gap in largest_gaps:
                    content.append(
                        f"- **{gap['competency']} - {gap['behavior']}:** {gap['difference']:.2f} vs. peer group (Evaluated by {gap['evaluator']})"
                    )
                content.append("")
            ...
            content.append(
                "1. Focus on being specific and practical rather than generic"
            )
            content.append(
                "2. Reference actual scores and data points from the evaluation"
            )
            content.append(
                "3. Tailor the feedback to this employee's specific role and level"
            )
            content.append(
                "4. Balance positive reinforcement with constructive development suggestions"
            )
            content.append(
                "5. Generate a complete feedback report following exactly the OUTPUT FORMAT specified above"
            )
            content.append("")
            content.append(
                "Your report should be professional, specific to this employee, and immediately actionable."
            )

            # Save prompt file
            output_file = (
                output_dir / f"ai_prompt_{person.replace(' ', '_')}_{timestamp}.md"
            )
            with open(output_file, "w", encoding="utf-8") as f:
                f.write("\n".join(content))
    ...
    def generate_stakeholder_comparison(self) -> dict:
        ...
        if not all_data:
            self.logger.warning(
                "No valid data found to generate stakeholder comparison"
            )
            return None
        ...
        for person in df["pessoa"].unique():
            ...
            content.append("")
            content.append(
                "This analysis compares evaluations from different stakeholders (manager, peers/partners, and self) against the peer group averages."
            )
            content.append("")
            ...
            summary_table.append(
                "| Stakeholder | Average Score | Peer Group Average | Difference |"
            )
            summary_table.append(
                "|------------|--------------|-------------------|-----------|"
            )

            stakeholder_avgs = latest_data.groupby("stakeholder_type")[
                "frequencia_colaborador"
            ].mean()
            group_avgs = latest_data.groupby("stakeholder_type")[
                "frequencia_grupo"
            ].mean()
            ...
            for stakeholder in ["manager", "peer", "self"]:
                if stakeholder in stakeholder_avgs:
                    ...
                    summary_table.append(
                        f"| {stakeholder.capitalize()} | {avg_score:.2f} | {group_avg:.2f} | {diff_str} |"
                    )
            ...
            overall_diff_str = (
                f"+{overall_diff:.2f}" if overall_diff > 0 else f"{overall_diff:.2f}"
            )

            content.append(
                f"**Overall Average:** {overall_avg:.2f} (Peer Group: {group_overall_avg:.2f}, Difference: {overall_diff_str})"
            )
            content.append("")
            ...
            for direcionador in latest_data["direcionador"].unique():
                ...
                comp_table = []
                comp_table.append(
                    "| Stakeholder | Behavior | Score | Peer Group | Diff |"
                )
                comp_table.append(
                    "|------------|----------|-------|------------|------|"
                )
                ...
                for comportamento in dir_data["comportamento"].unique():
                    ...
                    for stakeholder in ["manager", "peer", "self"]:
                        stakeholder_data = comp_data[
                            comp_data["stakeholder_type"] == stakeholder
                        ]
                        if not stakeholder_data.empty:
                            ...
                            diff_str = f"+{diff:.2f}" if diff > 0 else f"{diff:.2f}"
                            comp_table.append(
                                f"| {stakeholder.capitalize()} | {comportamento} | {score:.2f} | {group_score:.2f} | {diff_str} |"
                            )
            ...
            high_perf = latest_data.groupby(["direcionador", "comportamento"])[
                "frequencia_colaborador"
            ].mean()
            ...
            low_perf = latest_data.groupby(["direcionador", "comportamento"])[
                "frequencia_colaborador"
            ].mean()
            ...
            content.append("")
            content.append(
                "Areas with the largest differences between stakeholder perspectives:"
            )
            content.append("")
            ...
            for direcionador in latest_data["direcionador"].unique():
                for comportamento in latest_data[
                    latest_data["direcionador"] == direcionador
                ]["comportamento"].unique():
                    comp_data = latest_data[
                        (latest_data["direcionador"] == direcionador)
                        & (latest_data["comportamento"] == comportamento)
                    ]

                    stakeholder_scores = comp_data.groupby("stakeholder_type")[
                        "frequencia_colaborador"
                    ].mean()
                    if (
                        len(stakeholder_scores) > 1
                    ):  # Need at least 2 stakeholders to compare
            ...
            if gap_data:
                ...
                content.append(
                    "| Competency | Behavior | Gap Size | Highest Rating | Lowest Rating |"
                )
                content.append(
                    "|------------|----------|----------|---------------|---------------|"
                )
                ...
                for item in gap_data[:5]:  # Top 5 gaps
                    content.append(
                        f"| {item['direcionador']} | {item['comportamento']} | {item['gap']:.2f} | "
                ...
                content.append(
                    "Not enough data from different stakeholders to analyze perspective gaps."
                )
                content.append("")
            ...
            output_file = (
                output_dir
                / f"stakeholder_analysis_{person.replace(' ', '_')}_{timestamp}.md"
            )
            with open(output_file, "w", encoding="utf-8") as f:
                f.write("\n".join(content))
            ...
            self.logger.info(
                f"Stakeholder comparison report generated for {person}: {output_file}"
            )
    ...
    def generate_time_series_analysis(self) -> dict:
        ...
        for person in df["pessoa"].unique():
            ...
            if len(years) < 2:
                self.logger.info(
                    f"Skipping time series for {person}: not enough years of data"
                )
                continue
            ...
            content.append("")
            content.append(
                f"This analysis tracks performance trends for {person} over {len(years)} years ({min(years)} - {max(years)})."
            )
            content.append("")
            ...
            content.append(
                f'    title "Overall Performance Score by Year for {person}"'
            )
            ...
            for direcionador in direcionadores:
                ...
                content.append(
                    f"| {direcionador} | " + " | ".join(score_cells) + f" | {trend} |"
                )
            ...
            for direcionador in direcionadores:
                ...
                if len(dir_data["ano"].unique()) >= 2:
                    ...
                    first_score = dir_data[dir_data["ano"] == first_year][
                        "frequencia_colaborador"
                    ].mean()
                    last_score = dir_data[dir_data["ano"] == last_year][
                        "frequencia_colaborador"
                    ].mean()

                    change = last_score - first_score
                    improved_areas.append(
                        (direcionador, change, first_score, last_score)
                    )
            ...
            if improved_areas:
                ...
                for area, change, first, last in improved_areas[:3]:
                    if change > 0:
                        content.append(
                            f"- **{area}**: {first:.2f} → {last:.2f} (+{change:.2f})"
                        )
                content.append("")
            ...
            if len(all_years_data) >= 2:
                ...
                if total_change > 0.2:
                    content.append(
                        f"Overall, {person}'s performance has **improved** from {first_score:.2f} in {first_year} to {last_score:.2f} in {last_year} (+{total_change:.2f})."
                    )
                elif total_change < -0.2:
                    content.append(
                        f"Overall, {person}'s performance has **declined** from {first_score:.2f} in {first_year} to {last_score:.2f} in {last_year} ({total_change:.2f})."
                    )
                else:
                    content.append(
                        f"Overall, {person}'s performance has remained **stable** from {first_score:.2f} in {first_year} to {last_score:.2f} in {last_year} ({total_change:.2f})."
                    )

            # Save analysis file
            output_file = (
                output_dir / f"time_series_{person.replace(' ', '_')}_{timestamp}.md"
            )
            with open(output_file, "w", encoding="utf-8") as f:
                f.write("\n".join(content))
            ...
            report_files[person] = str(output_file)
            self.logger.info(
                f"Time series analysis generated for {person}: {output_file}"
            )
    ...
    def generate_radar_chart(self) -> dict:
        ...
        for person in df["pessoa"].unique():
            ...
            content.append("")
            content.append(
                f"This radar chart visualization shows {person}'s competency scores from different perspectives."
            )
            content.append(
                "The chart displays scores on a scale of 0-4 across all competency areas."
            )
            content.append("")

            # Create chart data by competency area
            competency_data = (
                latest_data.groupby("direcionador")["frequencia_colaborador"]
                .mean()
                .reset_index()
            )
            ...
            stakeholder_data = {}
            for stakeholder in ["manager", "peer", "self"]:
                stakeholder_latest = latest_data[
                    latest_data["stakeholder_type"] == stakeholder
                ]
                if not stakeholder_latest.empty:
                    temp_data = (
                        stakeholder_latest.groupby("direcionador")[
                            "frequencia_colaborador"
                        ]
                        .mean()
                        .reset_index()
            ...
            content.append(
                "        labels: ['"
                + "', '".join(competency_data["direcionador"])
                + "'],"
            )
            ...
            content.append(
                "                data: ["
                + ", ".join(
                    [f"{x:.2f}" for x in competency_data["frequencia_colaborador"]]
                )
                + "],"
            )
            content.append(
                f"                borderColor: '{colors['overall']['border']}',"
            )
            content.append(
                f"                backgroundColor: '{colors['overall']['background']}',"
            )
            ...
            for stakeholder, data in stakeholder_data.items():
                ...
                content.append(
                    "                data: ["
                    + ", ".join([f"{x:.2f}" for x in data["frequencia_colaborador"]])
                    + "],"
                )
                content.append(
                    f"                borderColor: '{colors[stakeholder]['border']}',"
                )
                content.append(
                    f"                backgroundColor: '{colors[stakeholder]['background']}',"
                )
                content.append("                borderWidth: 2")
                content.append("            },")
            ...
            peer_group_data = (
                latest_data.groupby("direcionador")["frequencia_grupo"]
                .mean()
                .reset_index()
            )
            ...
            content.append(
                "                data: ["
                + ", ".join([f"{x:.2f}" for x in peer_group_data["frequencia_grupo"]])
                + "],"
            )
            content.append(
                f"                borderColor: '{colors['peer_group']['border']}',"
            )
            content.append(
                f"                backgroundColor: '{colors['peer_group']['background']}',"
            )
            content.append("                borderWidth: 2,")
            ...
            content.append("")
            content.append(
                "> **Note**: The radar chart requires a browser with JavaScript enabled to view. If you're viewing this in a markdown viewer without JavaScript support, please open in a web browser."
            )
            content.append("")
            ...
            for i, row in competency_data.iterrows():
                ...
                peer_group = peer_group_data[
                    peer_group_data["direcionador"] == direcionador
                ]["frequencia_grupo"].values[0]
                gap = overall - peer_group
                gap_text = f"+{gap:.2f}" if gap > 0 else f"{gap:.2f}"

                content.append(
                    f"| {direcionador} | {overall:.2f} | {peer_group:.2f} | {gap_text} |"
                )
            ...
            if stakeholder_data:
                ...
                for direcionador in competency_data["direcionador"]:
                    ...
                    for stakeholder in stakeholder_data.keys():
                        if (
                            direcionador
                            in stakeholder_data[stakeholder]["direcionador"].values
                        ):
                            score = stakeholder_data[stakeholder][
                                stakeholder_data[stakeholder]["direcionador"]
                                == direcionador
                            ]["frequencia_colaborador"].values[0]
                            row.append(f"{score:.2f}")
                        else:
                            row.append("N/A")

                    peer_group_score = peer_group_data[
                        peer_group_data["direcionador"] == direcionador
                    ]["frequencia_grupo"].values[0]
            ...
            # Save analysis file
            output_file = (
                output_dir / f"radar_chart_{person.replace(' ', '_')}_{timestamp}.html"
            )
            with open(output_file, "w", encoding="utf-8") as f:
                f.write("\n".join(content))
    ...
    def generate_team_aggregation(self) -> str:
        ...
        content.append("")
        content.append(
            "This report provides an aggregated view of performance across teams/departments."
        )
        content.append(
            "It highlights collective strengths and development areas to enable targeted interventions."
        )
        content.append("")
        ...
        dept_avg = (
            latest_data.groupby("department")["frequencia_colaborador"]
            .agg(["mean", "count"])
            .reset_index()
        )
        ...
        for _, row in dept_avg.iterrows():
            content.append(
                f"| {row['department']} | {row['mean']:.2f} | {int(row['count'])} |"
            )
        ...
        for department in top_departments:
            ...
            comp_avg = (
                dept_data.groupby("direcionador")["frequencia_colaborador"]
                .mean()
                .reset_index()
            )
            ...
            for _, row in comp_avg.iterrows():
                content.append(
                    f'    "{row["direcionador"]}" {row["frequencia_colaborador"]:.2f}'
                )
            ...
            for i, (_, row) in enumerate(comp_avg.iterrows()):
                content.append(
                    f"| {row['direcionador']} | {row['frequencia_colaborador']:.2f} | {i + 1} |"
                )
        ...
        for competency in top_competencies:
            ...
            dept_comp_avg = (
                comp_data.groupby("department")["frequencia_colaborador"]
                .mean()
                .reset_index()
            )
            dept_comp_avg = dept_comp_avg.sort_values(
                "frequencia_colaborador", ascending=False
            )
            ...
            for _, row in dept_comp_avg.iterrows():
                content.append(
                    f'    "{row["department"]}" {row["frequencia_colaborador"]:.2f}'
                )
            ...
            for _, row in dept_comp_avg.iterrows():
                ...
                content.append(
                    f"| {row['department']} | {dept_score:.2f} | {diff_text} |"
                )
        ...
        content.append("")
        content.append(
            "This table shows the relative performance of departments across different competency areas:"
        )
        content.append("")
        ...
        content.append(
            "| Department | " + " | ".join(top_competencies) + " | Overall |"
        )
        content.append(
            "|"
            + "-" * 12
            + "|"
            + "".join(["-" * 12 + "|" for _ in top_competencies])
            + "-" * 9
            + "|"
        )
        ...
        content.append("")
        content.append(
            "**Legend**: 🟢 Strong (≥3.5) | 🟡 Satisfactory (2.5-3.5) | 🔴 Needs Improvement (<2.5)"
        )
        content.append("")
        ...
        content.append(
            "Based on the aggregated data, the following organization-wide focus areas are recommended:"
        )
        content.append("")
        ...
        for comp in bottom_competencies:
            ...
            content.append(
                "   - Consider organization-wide training programs focused on this area"
            )
            content.append(
                "   - Identify and leverage high performers as internal coaches"
            )
            content.append(
                "   - Set specific team-level improvement goals for this competency"
            )
            content.append("")
    ...
    def generate_benchmark_report(self) -> dict:
        ...
        benchmarks = {
            ...
            "Director": {"threshold": 4.0, "next_level": None, "next_threshold": None},
            "Executive": {"threshold": 4.0, "next_level": None, "next_threshold": None},
        }
        ...
        for person in df["pessoa"].unique():
            ...
            if not levels.size:
                self.logger.warning(
                    f"Skipping benchmark for {person}: missing level information"
                )
                continue
            ...
            if current_level not in benchmarks:
                if (
                    "junior" in current_level.lower()
                    or "trainee" in current_level.lower()
                ):
                    level_key = "Junior"
                elif (
                    "mid" in current_level.lower()
                    or "intermediate" in current_level.lower()
                ):
                    level_key = "Mid-level"
                elif "senior" in current_level.lower():
                    level_key = "Senior"
                elif (
                    "principal" in current_level.lower()
                    or "staff" in current_level.lower()
                ):
                    level_key = "Principal"
                elif (
                    "director" in current_level.lower()
                    or "manager" in current_level.lower()
                ):
                    level_key = "Director"
            ...
            if level_key in benchmarks:
                ...
                self.logger.warning(
                    f"Using default benchmarks for {person}: level {level_key} not found"
                )
            ...
            content.append("")
            content.append(
                f"This report compares {person}'s performance against expected benchmarks for their current level ({current_level}) and the requirements for the next level."
            )
            content.append("")
            ...
            content.append(
                f"**Status:** {'✅ Meets' if meets_benchmark else '❌ Below'} benchmark for current level"
            )
            content.append("")
            ...
            if benchmark["next_level"]:
                ...
                content.append(f"**Overall Score:** {overall_score:.2f}")
                content.append(
                    f"**Status:** {'✅ Ready' if ready_for_next else '❌ Not yet ready'} for {next_level}"
                )
                content.append("")
            ...
            for _ in comp_avg.iterrows():
                content.append(
                    f'      "{row["direcionador"]}" {benchmark["threshold"]:.2f}'
                )
            ...
            if benchmark["next_level"]:
                ...
                for _ in comp_avg.iterrows():
                    content.append(
                        f'      "{row["direcionador"]}" {benchmark["next_threshold"]:.2f}'
                    )
            ...
            content.append(
                "| Competency | Your Score | Current Benchmark | Status | Next Level Benchmark | Gap |"
            )
            content.append(
                "|------------|------------|-------------------|--------|---------------------|-----|"
            )
            ...
            for _, row in comp_avg.iterrows():
                ...
                if benchmark["next_level"]:
                    ...
                    content.append(
                        f"| {direcionador} | {score:.2f} | {benchmark['threshold']:.2f} | {status} | {next_threshold:.2f} | {gap_text} |"
                    )
                else:
                    content.append(
                        f"| {direcionador} | {score:.2f} | {benchmark['threshold']:.2f} | {status} | — | — |"
                    )
            ...
            if benchmark["next_level"]:
                ...
                comp_avg["gap"] = (
                    benchmark["next_threshold"] - comp_avg["frequencia_colaborador"]
                )
                comp_gaps = comp_avg[comp_avg["gap"] > 0].sort_values(
                    "gap", ascending=False
                )

                if not comp_gaps.empty:
                    content.append(
                        f"To reach the {benchmark['next_level']} level, focus on improving these competencies:"
                    )
                    content.append("")

                    for _, row in comp_gaps.iterrows():
                        content.append(
                            f"- **{row['direcionador']}**: Current score {row['frequencia_colaborador']:.2f}, "
                    ...
                    content.append(
                        f"You're meeting all competency benchmarks for the {benchmark['next_level']} level!"
                    )
                    content.append("")
            ...
            peer_avg["diff"] = (
                peer_avg["frequencia_colaborador"] - peer_avg["frequencia_grupo"]
            )
            ...
            for _, row in peer_avg.iterrows():
                ...
                diff_text = f"+{diff:.2f}" if diff > 0 else f"{diff:.2f}"
                content.append(
                    f"| {row['direcionador']} | {row['frequencia_colaborador']:.2f} | {row['frequencia_grupo']:.2f} | {diff_text} |"
                )
            ...
            if meets_benchmark:
                content.append(
                    f"✅ **Overall Assessment**: You are meeting the performance expectations for your current level ({current_level})."
                )
            else:
                content.append(
                    f"❌ **Overall Assessment**: You are currently below the benchmark for your level ({current_level})."
                )
            content.append("")
            ...
            if benchmark["next_level"]:
                if ready_for_next:
                    content.append(
                        f"✅ **Next Level Readiness**: Your performance meets the benchmarks for {benchmark['next_level']}."
                    )
                else:
                    content.append(
                        f"❓ **Next Level Readiness**: You have a gap of {(next_threshold - overall_score):.2f} to reach the benchmark for {benchmark['next_level']}."
                    )
                content.append("")

            # Add top strengths
            top_strengths = peer_avg.sort_values(
                "frequencia_colaborador", ascending=False
            ).head(3)
            ...
            if benchmark["next_level"] and not ready_for_next:
                ...
                for _, row in top_gaps.iterrows():
                    content.append(
                        f"- {row['direcionador']} (current: {row['frequencia_colaborador']:.2f}, target: {benchmark['next_threshold']:.2f})"
                    )
                content.append("")

            # Save report file
            output_file = (
                output_dir / f"benchmark_{person.replace(' ', '_')}_{timestamp}.md"
            )
            with open(output_file, "w", encoding="utf-8") as f:
                f.write("\n".join(content))
    ...
    def generate_heat_map(self) -> dict:
        ...
        for person in df["pessoa"].unique():
            ...
            content.append("")
            content.append(
                "This heat map visualization shows performance scores across different competencies and behaviors."
            )
            content.append(
                "The colors indicate performance level (red = low, yellow = medium, green = high)."
            )
            content.append("")
            ...
            for direcionador in direcionadores:
                ...
                content.append("")
                content.append(
                    "| Behavior | Manager | Self | Peer | Overall | Group Avg |"
                )
                content.append(
                    "|----------|---------|------|------|---------|-----------|"
                )
            ...
            comp_data = (
                latest_data.groupby(["direcionador", "stakeholder_type"])[
                    "frequencia_colaborador"
                ]
                .mean()
                .reset_index()
            ...
            # Create table
            content.append(
                "| Competency | Manager | Self | Peer | Overall | Group Avg |"
            )
            content.append(
                "|------------|---------|------|------|---------|-----------|"
            )
            ...
            for direcionador in direcionadores:
                ...
                # Manager score
                manager_row = comp_data[
                    (comp_data["direcionador"] == direcionador)
                    & (comp_data["stakeholder_type"] == "manager")
                ]
                ...
                self_row = comp_data[
                    (comp_data["direcionador"] == direcionador)
                    & (comp_data["stakeholder_type"] == "self")
                ]
                ...
                peer_row = comp_data[
                    (comp_data["direcionador"] == direcionador)
                    & (comp_data["stakeholder_type"] == "peer")
                ]
            ...
            # Save heat map file
            output_file = (
                output_dir / f"heat_map_{person.replace(' ', '_')}_{timestamp}.md"
            )
            with open(output_file, "w", encoding="utf-8") as f:
                f.write("\n".join(content))
    ...
    def generate_natural_language_summary(self) -> dict:
        ...
        if not all_data:
            self.logger.warning(
                "No valid data found to generate natural language summaries"
            )
            return None
        ...
        for person in df["pessoa"].unique():
            ...
            # Get job info
            job_title = (
                latest_data["cargo"].iloc[0]
                if "cargo" in latest_data
                else "Unknown Role"
            )
            job_level = (
                latest_data["nivel"].iloc[0]
                if "nivel" in latest_data
                else "Unknown Level"
            )

            # Get overall assessment
            overall_assessment = (
                latest_data["conceito"].iloc[0]
                if "conceito" in latest_data
                else "Unknown"
            )
            ...
            comp_scores = latest_data.groupby("direcionador")[
                "frequencia_colaborador"
            ].mean()
            ...
            strongest_peer = latest_data[latest_data["direcionador"] == strongest_comp][
                "frequencia_grupo"
            ].mean()
            ...
            if strength_diff > 0.5:
                ...
                strengths += (
                    f"Their {strongest_comp} skills are above the peer group average. "
                )

            summary.append(strengths)

            # Development areas paragraph
            development = "Areas for potential development include "
            ...
            weakest_peer = latest_data[latest_data["direcionador"] == weakest_comp][
                "frequencia_grupo"
            ].mean()
            ...
            stakeholder_data = latest_data.groupby("stakeholder_type")[
                "frequencia_colaborador"
            ].mean()
            ...
            if overall_score >= 3.5:
                ...
                conclusion += (
                    f"{person} has opportunities for improvement in several areas. "
                )
            ...
            output_file = (
                output_dir
                / f"natural_summary_{person.replace(' ', '_')}_{timestamp}.md"
            )
            with open(output_file, "w", encoding="utf-8") as f:
                f.write("\n".join(content))
            ...
            report_files[person] = str(output_file)
            self.logger.info(
                f"Natural language summary generated for {person}: {output_file}"
            )
    ...
    def generate_action_plan(self) -> dict:
        ...
        for person in df["pessoa"].unique():
            ...
            content.append("")
            content.append(
                f"This action plan is based on {person}'s performance evaluation data and focused on key development areas."
            )
            content.append(
                "The plan includes specific actions, resources, and timeframes to support growth in targeted competencies."
            )
            content.append("")
            ...
            for i, (_, row) in enumerate(focus_areas.iterrows()):
                ...
                content.append(
                    f"### {i + 1}. {direcionador} (Current Score: {score:.2f})"
                )
                content.append("")
                content.append(
                    f"**Target**: Improve {direcionador} skills to meet or exceed peer group average."
                )
                content.append("")
                ...
                if direcionador in development_suggestions:
                    ...
                    for key in development_suggestions:
                        if (
                            key.lower() in direcionador.lower()
                            or direcionador.lower() in key.lower()
                        ):
            ...
            content.append("")
            content.append(
                "I commit to following this development plan to the best of my ability."
            )
            content.append("")
            content.append(
            ...
            output_file = (
                output_dir / f"action_plan_{person.replace(' ', '_')}_{timestamp}.md"
            )
            with open(output_file, "w", encoding="utf-8") as f:
                f.write("\n".join(content))
    ...
    def generate_individual_report(self) -> dict:
        ...
        for person in df["pessoa"].unique():
            ...
            stakeholder_scores = latest_data.groupby("avaliador")[
                "frequencia_colaborador"
            ].mean()
            ...
            for stakeholder in latest_data["avaliador"].unique():
                ...
                for direcionador in stakeholder_data["direcionador"].unique():
                    dir_data = stakeholder_data[
                        stakeholder_data["direcionador"] == direcionador
                    ]
            ...
            for gap in stakeholder_gaps[:5]:  # Top 5 gaps
                content.append(
                    f"| {gap['direcionador']} | {gap['stakeholder']} | {gap['score']:.2f} | {gap['group_avg']:.2f} | {gap['gap']:.2f} |"
                )
            ...
            output_file = (
                output_dir
                / f"individual_report_{person.replace(' ', '_')}_{timestamp}.md"
            )
            with open(output_file, "w", encoding="utf-8") as f:
                f.write("\n".join(content))
        ...
        for csv_file in csv_files:
            try:
                ...
                for _, row in df.iterrows():
                    ...
                    report_file = (
                        reports_dir
                        / f"{row['name']}_report_{datetime.now().strftime('%Y%m%d')}.md"
                    )
