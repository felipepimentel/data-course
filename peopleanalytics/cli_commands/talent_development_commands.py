...
# Import BaseCommand from the parent module
...
class TalentDevelopmentCommands:
    ...
    def _handle_nine_box(self, args):
        ...
        self.console.print(
            Panel(
                "[yellow]Comando da Matriz 9-Box incompleto.[/yellow] "
    ...
    def _handle_nine_box_visualize(self, args, matrix: DynamicMatrix9Box):
        ...
        """
        self.console.print(
            f"[bold]Visualizando Matriz 9-Box para:[/bold] {args.person_id}"
        )
    ...
    def _handle_nine_box_report(self, args, matrix: DynamicMatrix9Box):
        ...
        """
        self.console.print(
            f"[bold]Gerando Relatório de Matriz 9-Box para:[/bold] {args.person_id}"
        )
        ...
        try:
            ...
            report_path, viz_path = matrix.generate_report(
                args.person_id, output_path=output_path, timespan_quarters=args.quarters
            )
            ...
            try:
                ...
                self.console.print(
                    f"[yellow]Não foi possível mostrar prévia do relatório:[/yellow] {str(e)}"
                )
    ...
    def _handle_nine_box_add_position(self, args, matrix: DynamicMatrix9Box):
        ...
        """
        self.console.print(
            f"[bold]Adicionando posição na Matriz 9-Box para:[/bold] {args.person_id}"
        )

        # Validar valores
        if not 0 <= args.performance <= 10:
            self.console.print(
                "[red]Erro: O valor de desempenho deve estar entre 0 e 10.[/red]"
            )
            return

        if not 0 <= args.potential <= 10:
            self.console.print(
                "[red]Erro: O valor de potencial deve estar entre 0 e 10.[/red]"
            )
            return
        ...
        if args.date:
            try:
                ...
                self.console.print(
                    "[red]Erro: Formato de data inválido. Use YYYY-MM-DD.[/red]"
                )
                return
        else:
        ...
        try:
            ...
            position = matrix.add_position(
                args.person_id, args.performance, args.potential, date, source_data
            )
            ...
            if self.data_pipeline:
                ...
                self.console.print(
                    "[green]Posição salva com sucesso no banco de dados.[/green]"
                )
            ...
            self.console.print(
                f"   [green]python -m peopleanalytics nine-box visualize {args.person_id} --show-future[/green]"
            )
            self.console.print("2. Gerar relatório completo:")
            self.console.print(
                f"   [green]python -m peopleanalytics nine-box report {args.person_id}[/green]"
            )
    ...
    def _handle_feedback_cycle(self, args):
        ...
        self.console.print(
            Panel(
                "[yellow]Funcionalidade Ciclo de Feedback em desenvolvimento.[/yellow]",
                title="Em Construção",
            )
        )
        return True
    ...
    def _handle_influence_network(self, args):
        ...
        self.console.print(
            Panel(
                "[yellow]Funcionalidade Rede de Influência em desenvolvimento.[/yellow]",
                title="Em Construção",
            )
        )
        return True
    ...
    def _handle_career_simulation(self, args):
        ...
        self.console.print(
            Panel(
                "[yellow]Funcionalidade Simulação de Carreira em desenvolvimento.[/yellow]",
                title="Em Construção",
            )
        )
        return True
...
class NineBoxCommand(BaseCommand):
    """Command for working with Nine-Box talent assessment matrix."""

    def __init__(self):
        """Initialize the NineBoxCommand."""
        super().__init__()
        self.console = None
        self.data_pipeline = None
        self.talent_commands = None
    ...
    def add_arguments(self, parser):
        ...
        """
        # Add subparsers for nine-box commands
        subparsers = parser.add_subparsers(dest="nine_box_command")
    ...
    def execute(self, args):
        ...
        if self.talent_commands is None:
            self.talent_commands = TalentDevelopmentCommands(
                self.console, self.data_pipeline
            )

        # Execute the command through the TalentDevelopmentCommands instance
        args.command = "nine-box"
        success = self.talent_commands.handle_command(args)
