...
class DynamicMatrix9Box:
    ...
    def add_position(self, person_id: str, performance: float, potential: float, 
                    timestamp: datetime.datetime, source_data: Dict[str, Any] = None) -> MatrixPosition:
        """
    ...
    def analyze_trajectory(self, person_id: str, timespan_quarters: int = 8) -> Dict[str, Any]:
        """
        ...
        cutoff_date = datetime.datetime.now() - datetime.timedelta(days=timespan_quarters*90)
        positions = [p for p in self.historical_data[person_id] 
    ...
    def visualize_matrix(self, person_id: str, output_path: Optional[Path] = None, 
                         show_trajectories: bool = True, show_future: bool = True,
                         timespan_quarters: int = 8) -> Path:
        """
        ...
        for i in range(3):
            for j in range(3):
                rect = plt.Rectangle((i*3.33, j*3.33), 3.33, 3.33, 
                                    facecolor=colors[i+j*3], alpha=0.3)
                ax.add_patch(rect)
                
                # Adicionar rótulos nos quadrantes
                labels = [
                    "Problema de\nDesempenho", "Enigma", "Estrela em\nAscensão",
                    "Profissional\nSólido", "Profissional\nChave", "Futuro\nTalento",
                    "Especialista\nTécnico", "Alta\nPerformance", "Estrela\nOrganizacional"
                ]
                ax.text(i*3.33+1.67, j*3.33+1.67, labels[i+j*3], 
        ...
        if show_trajectories and len(positions) > 0:
            ...
            for i, (perf, pot, ts, sz, clr) in enumerate(zip(perf_values, pot_values, 
                                                         timestamps, sizes, colors)):
                ...
                if i > 0:
                    ax.plot([perf_values[i-1], perf], [pot_values[i-1], pot], 
                           'k-', alpha=0.5)
            ...
            cbar.set_ticklabels([timestamps[0].strftime("%Y-Q%m"), 
                                timestamps[-1].strftime("%Y-Q%m")])
        ...
        if show_future and analysis['future_projection']:
            ...
            ax.scatter(future_perf, future_pot, s=30, color='red', alpha=0.5, 
                      marker='x')
            ...
            for i, (perf, pot) in enumerate(zip(future_perf, future_pot)):
                ...
                circle = plt.Circle((perf, pot), confidence_factor, color='red', 
                                   fill=True, alpha=0.1)
                ax.add_patch(circle)
        ...
        if output_path:
            ...
            output_file = output_dir / f"{person_id}_matrix_9box_{datetime.datetime.now().strftime('%Y%m%d')}.png"
            plt.savefig(output_file)
            plt.close()
            return output_file
            
    def generate_report(self, person_id: str, output_path: Optional[Path] = None,
                       timespan_quarters: int = 8) -> Tuple[Path, Path]:
        """
        ...
        if output_path:
            ...
            output_dir = Path("output") / "matrix_9box"
            output_dir.mkdir(parents=True, exist_ok=True)
            viz_path = output_dir / f"{person_id}_matrix_9box_{datetime.datetime.now().strftime('%Y%m%d')}.png"
        ...
        if output_path:
            ...
            report_path = Path("output") / "matrix_9box" / f"{person_id}_matrix_9box_report_{datetime.datetime.now().strftime('%Y%m%d')}.md"
        
        with open(report_path, "w") as f:
            f.write(f"# Relatório de Matriz 9-Box: {person_id}\n\n")
            f.write(f"Data de geração: {datetime.datetime.now().strftime('%d/%m/%Y')}\n\n")
            ...
            if positions:
                ...
                f.write(f"**Data da última avaliação:** {current.timestamp.strftime('%d/%m/%Y')}\n\n")
            else:
                f.write("Não há dados de posicionamento disponíveis.\n\n")
            
            f.write("## Trajetória\n\n")
            if movement_vector:
                f.write(f"**Direção do movimento:** {movement_vector.direction_degrees:.1f}° ")
                ...
                f.write(f"**Velocidade de evolução:** {movement_vector.velocity:.2f} pontos/trimestre\n")
            ...
            if acceleration_triggers:
                ...
                f.write("Nenhum gatilho de aceleração identificado no período analisado.\n\n")
            
            f.write("## Projeção Futura\n\n")
            if future_projection:
                f.write(f"**Quadrante projetado (em 1 ano):** {future_projection.projected_positions[-1].quadrant_name}\n")
                f.write(f"**Desempenho projetado:** {future_projection.projected_positions[-1].performance:.2f}/10\n")
                f.write(f"**Potencial projetado:** {future_projection.projected_positions[-1].potential:.2f}/10\n")
                f.write(f"**Confiança da projeção:** {future_projection.confidence_level*100:.0f}%\n\n")
                
                f.write("### Intervenções Recomendadas\n\n")
                for i, intervention in enumerate(future_projection.recommended_interventions, 1):
                    f.write(f"{i}. **{intervention['title']}**\n")
                    f.write(f"   {intervention['description']}\n")
                    f.write(f"   Impacto estimado: {intervention['estimated_impact']}\n\n")
            else:
            ...
            if positions:
                current = positions[-1]
                quads = {
                    (0, 0): "Considerar programa de recuperação de desempenho ou reavaliação de fit no cargo atual.",
                    (0, 1): "Investigar barreiras ao desempenho. Considerar realocação ou mudança de responsabilidades.",
                    (0, 2): "Potencial não realizado. Avaliar se está na função correta ou precisa de melhor direcionamento.",
                    (1, 0): "Bom executor, mas com limitações de crescimento. Foco em especialização técnica.",
                    (1, 1): "Profissional sólido. Manter engajado com desafios incrementais.",
                    (1, 2): "Alto potencial em desenvolvimento. Oferecer desafios crescentes e mentoria estruturada.",
                    (2, 0): "Excelente no papel atual, mas com teto atingido. Valorizar contribuição especializada.",
                    (2, 1): "Forte performer com potencial de crescimento. Expandir escopo e responsabilidades.",
                    (2, 2): "Talento excepcional. Desenvolvimento acelerado, exposição estratégica e plano de sucessão."
                }
                ...
                if movement_vector:
                    if movement_vector.velocity < 0:
                        f.write("**Alerta:** A trajetória atual mostra retrocesso. Recomenda-se intervenção imediata e análise aprofundada das causas.\n\n")
                    elif movement_vector.velocity < 0.3:
                        f.write("**Atenção:** A velocidade de evolução está abaixo do esperado. Revisar desafios e suporte oferecidos.\n\n")
            else:
