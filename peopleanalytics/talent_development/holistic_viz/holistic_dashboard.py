...
@dataclass
class TeamSummary:
    """Resumo holístico de uma equipe."""
    ...
    high_performers: List[str]  # Lista de IDs de pessoas
    development_needed: List[str]  # Lista de IDs de pessoas
    skill_coverage: Dict[str, float]  # Habilidade -> nível médio
    skill_gaps: List[str]
    team_network_density: float
    key_influencers: List[str]  # Lista de IDs de pessoas
    team_trends: Dict[str, List[float]]
...
class HolisticDashboard:
    ...
    def _integrate_performance_data(self):
        ...
        for person_id, person_metrics in metrics_by_person.items():
            ...
            for metric_type in set(m.metric_type for m in person_metrics):
                type_metrics = [
                    m for m in sorted_metrics if m.metric_type == metric_type
                ]
                if type_metrics:
    ...
    def _integrate_career_data(self):
        ...
        for person_id, paths in self.career_simulator.simulated_paths.items():
            ...
            if (
                current_position_id
                and current_position_id in self.career_simulator.positions
            ):
    ...
    def _integrate_network_data(self):
        ...
        # Integrar por pessoa
        for person_id in (
            list(centrality.keys())
            + list(impact_multipliers.keys())
            + list(social_capital.keys())
        ):
            if person_id not in self.person_data:
                self.person_data[person_id] = {
                    "basic_info": {"id": person_id},
    ...
    def _generate_person_summaries(self):
        ...
        for person_id, data in self.person_data.items():
            ...
            if career_data.get("paths"):
                ...
                sorted_paths = sorted(
                    career_data["paths"], key=lambda p: p.growth_score, reverse=True
                )
            ...
            if career_data.get("skills"):
                ...
                sorted_skills = sorted(
                    career_data["skills"].items(), key=lambda x: x[1], reverse=True
                )[:5]
            ...
            if career_data.get("skill_gaps"):
                ...
                sorted_gaps = sorted(
                    career_data["skill_gaps"].items(), key=lambda x: x[1], reverse=True
                )[:3]
    ...
    def _generate_recommendations(self, person_id: str) -> List[str]:
        ...
        if skill_gaps:
            ...
            recommendations.append(
                f"Desenvolver a habilidade {top_gap[0]} para aumentar adequação à posição atual."
            )
        ...
        if potential_score > performance_score + 0.2:
            recommendations.append(
                "Focar em converter alto potencial em resultados concretos."
            )
        elif performance_score > potential_score + 0.2:
            recommendations.append(
                "Buscar novos desafios que permitam expandir o potencial além da zona de conforto."
            )
        ...
        if centrality < 0.3:
            recommendations.append(
                "Ampliar rede de conexões para aumentar impacto e influência organizacional."
            )
        ...
        if paths:
            ...
            if len(best_path.positions) > 1:
                ...
                recommendations.append(
                    f"Preparar-se para {pos_title} adquirindo experiências estratégicas."
                )
    ...
    def _calculate_organization_metrics(self):
        ...
        for person_id, summary in self.person_summaries.items():
            ...
            self.organization_metrics["talent_distribution"][
                summary.nine_box_position
            ] += 1
            ...
            for skill, level in summary.key_skills.items():
                if skill not in self.organization_metrics["skill_coverage"]:
                    self.organization_metrics["skill_coverage"][skill] = {
                        "count": 0,
                        "avg_level": 0,
                    }
    ...
    def create_person_dashboard(
        self, person_id: str, output_path: Optional[Path] = None
    ) -> Optional[Path]:
        """
        ...
        try:
            ...
            fig.suptitle(
                f"Dashboard Holístico de Desenvolvimento de Talentos - {summary.name}",
                fontsize=16,
                y=0.98,
            )
    ...
    def _plot_skills_radar(self, fig: Figure, position, summary: PersonSummary):
        ...
        if not skills:
            ax.text(
                0.5,
                0.5,
                "Dados insuficientes para radar de competências",
                ha="center",
                va="center",
    ...
    def _plot_matrix_trajectory(self, fig: Figure, position, summary: PersonSummary):
        ...
        trajectory = (
            self.person_data.get(summary.person_id, {})
            .get("potential", {})
            .get("trajectory", [])
        )
    ...
    def _plot_influence_network(self, fig: Figure, position, summary: PersonSummary):
        ...
        if not self.influence_network:
            ax.text(
                0.5,
                0.5,
                "Dados de rede de influência não disponíveis",
                ha="center",
                va="center",
                transform=ax.transAxes,
            )
        ...
        if not person_network or person_network.number_of_nodes() <= 1:
            ax.text(
                0.5,
                0.5,
                "Rede de influência insuficiente para visualização",
                ha="center",
                va="center",
                transform=ax.transAxes,
            )
    ...
    def _plot_performance_trends(self, fig: Figure, position, summary: PersonSummary):
        ...
        for i, metric in enumerate(metrics):
            ...
            x = range(len(values))
            ax.plot(x, values, marker="o", color=colors[i % len(colors)], label=metric)
    ...
    def _plot_career_simulation(self, fig: Figure, position, summary: PersonSummary):
        ...
        if not paths:
            ax.text(
                0.5,
                0.5,
                "Dados de simulação de carreira não disponíveis",
                ha="center",
                va="center",
                transform=ax.transAxes,
            )
        ...
        if not positions:
            ax.text(
                0.5,
                0.5,
                "Caminho de carreira sem posições definidas",
                ha="center",
                va="center",
                transform=ax.transAxes,
            )
        ...
        levels = [
            self.career_simulator.positions[pos_id].level
            for pos_id, _ in best_path.positions
        ]
        colors = plt.cm.viridis(np.array(levels) / max(levels))

        # Plotar barras horizontais
        for i, (pos, duration, start, color) in enumerate(
            zip(positions, durations, start_times, colors)
        ):
        ...
        info_text = (
            f"Score de Crescimento: {best_path.growth_score:.2f}\n"
    ...
    def generate_holistic_report(
        ...
        try:
            ...
            with open(output_path, "w", encoding="utf-8") as f:
                f.write("# RELATÓRIO HOLÍSTICO DE DESENVOLVIMENTO\n\n")
                ...
                f.write("## Análise Multidimensional\n\n")
                ...
                f.write("### Potencial e Desempenho\n\n")
                ...
                f.write("### Análise de Competências\n\n")
                f.write("**Competências Principais**:\n")
                for skill, level in summary.key_skills.items():
                ...
                f.write("\n### Análise de Carreira\n\n")
                f.write(
                    f"**Score de Crescimento de Carreira**: {summary.career_growth_score:.2f}\n"
                )
                ...
                if paths:
                    sorted_paths = sorted(
                        paths, key=lambda p: p.growth_score, reverse=True
                    )
                    best_path = sorted_paths[0]
                    ...
                    for i, (pos_id, duration) in enumerate(best_path.positions):
                        if pos_id in self.career_simulator.positions:
                            position = self.career_simulator.positions[pos_id]
                            f.write(
                                f"{i + 1}. {position.title} ({position.track}) - {duration} anos\n"
                            )
                ...
                f.write("### Previsões de Desempenho\n\n")
                if summary.predictions:
                    for metric, pred in summary.predictions.items():
                        ...
                        if "confidence_interval" in pred:
                            ci = pred["confidence_interval"]
                            f.write(
                                f"- Intervalo de Confiança: [{ci[0]:.2f}, {ci[1]:.2f}]\n"
                            )
                else:
                    f.write("Previsões não disponíveis.\n")
                ...
                f.write("\n## Próximos Passos\n\n")
                f.write("1. Revisar plano de desenvolvimento pessoal\n")
                f.write("2. Agendar reunião de feedback com gestor\n")
                f.write(
                    "3. Definir objetivos de desenvolvimento para os próximos 6 meses\n"
                )
    ...
    def generate_team_report(
        self, team_id: str, person_ids: List[str], output_path: Optional[Path] = None
    ) -> Optional[Path]:
        """
        ...
        try:
            ...
            for person_id in available_persons:
                ...
                for skill, level in summary.key_skills.items():
                    if skill not in team_metrics["skill_coverage"]:
                        team_metrics["skill_coverage"][skill] = {
                            "count": 0,
                            "avg_level": 0,
                        }
            ...
            # Gerar relatório
            with open(output_path, "w", encoding="utf-8") as f:
                f.write("# RELATÓRIO HOLÍSTICO DE EQUIPE\n\n")
                ...
                f.write("## Métricas da Equipe\n\n")
                f.write(
                    f"**Performance Média**: {team_metrics['avg_performance']:.2f}\n"
                )
                f.write(f"**Potencial Médio**: {team_metrics['avg_potential']:.2f}\n")
                f.write(f"**Crescimento Médio**: {team_metrics['avg_growth']:.2f}\n")
                f.write(
                    f"**Influência Média**: {team_metrics['avg_influence']:.2f}\n\n"
                )

                f.write("## Distribuição de Talentos\n\n")
                f.write("| Quadrante | Quantidade | Porcentagem |\n")
                f.write("|-----------|------------|-------------|\n")
                ...
                f.write("| Competência | Cobertura | Nível Médio |\n")
                f.write("|-------------|-----------|-------------|\n")

                for skill, data in sorted_skills[:10]:  # Top 10
                    coverage = (data["count"] / num_persons) * 100
                    f.write(
                        f"| {skill} | {coverage:.1f}% | {data['avg_level']:.2f} |\n"
                    )

                f.write("\n## Áreas de Desenvolvimento\n\n")
                f.write("| Área | Frequência |\n")
                f.write("|------|------------|\n")
                ...
                for person_id in available_persons:
                    ...
                    f.write(
                        f"- **Áreas de Desenvolvimento**: {', '.join(summary.development_areas)}\n\n"
                    )

                f.write("\n## Recomendações para a Equipe\n\n")

                # Gerar recomendações baseadas nos dados da equipe
                f.write("1. **Desenvolvimento de Competências**: ")
                if sorted_areas:
                    f.write(f"Focar no desenvolvimento de {sorted_areas[0][0]}, ")
                    if len(sorted_areas) > 1:
                        f.write(
                            "que é uma área de desenvolvimento comum para vários membros.\n"
                        )
                    else:
                        f.write("que é uma área de desenvolvimento prioritária.\n")
                else:
                    f.write("Desenvolver um plano estruturado de capacitação.\n")

                f.write("2. **Balanceamento de Talentos**: ")
                high_performers = (
                    team_metrics["talent_distribution"][(3, 3)]
                    + team_metrics["talent_distribution"][(2, 3)]
                    + team_metrics["talent_distribution"][(3, 2)]
                )
                high_potential = (
                    team_metrics["talent_distribution"][(3, 1)]
                    + team_metrics["talent_distribution"][(3, 2)]
                )

                if high_performers < num_persons * 0.2:
                    f.write(
                        "Considerar integrar mais pessoas de alta performance à equipe.\n"
                    )
                elif high_potential < num_persons * 0.2:
                    f.write(
                        "Buscar talentos com alto potencial para renovação da equipe.\n"
                    )
                else:
                    f.write(
                        "A equipe tem um bom balanceamento entre performance e potencial.\n"
                    )

                f.write(
                    "3. **Fortalecimento da Rede de Influência**: Promover mais colaboração entre membros "
                )
                if team_metrics["avg_influence"] < 0.4:
                    f.write(
                        "para aumentar o impacto coletivo, que está abaixo do ideal.\n"
                    )
                else:
                    f.write("para manter o bom nível de influência organizacional.\n")
