...
class Sync:
    ...
    def _sync_individual_reports(self) -> List[str]:
        ...
        for report_file in report_files:
            try:
                ...
                dest_file = (
                    person_dir / f"individual_report_{report_file.name.split('_')[-1]}"
                )
                shutil.copy2(report_file, dest_file)

                results.append(
                    f"Synced {report_file.name} to {person_name}'s directory"
                )
    ...
    def _process_manager_feedback(self) -> List[str]:
        ...
        for feedback_file in feedback_files:
            try:
                ...
                feedback_report = self.manager_feedback.generate_feedback_report(
                    person_name
                )
                if feedback_report:
                    # Create person's directory if it doesn't exist
                    ...
                    report_file = (
                        person_dir
                        / f"manager_feedback_report_{datetime.now().strftime('%Y%m%d')}.md"
                    )
                    with open(report_file, "w") as f:
                        f.write(feedback_report)

                    results.append(
                        f"Generated manager feedback report for {person_name}"
                    )

                    # Export to Excel for data analysis
                    excel_path = self.manager_feedback.export_feedback_to_excel(
                        person_name
                    )
                    if excel_path:
                ...
                results.append(
                    f"Error processing feedback for {feedback_file.stem}: {str(e)}"
                )
        ...
        for person_dir in person_dirs:
            try:
                ...
                if individual_reports and manager_reports:
                    ...
                    pdi = self._create_pdi(
                        person_name, individual_report, manager_report, career_report
                    )
                    pdi_file = (
                        person_dir
                        / f"personal_development_plan_{datetime.now().strftime('%Y%m%d')}.md"
                    )
                ...
                results.append(
                    f"Error generating comprehensive report for {person_dir.name}: {str(e)}"
                )
    ...
    def _process_career_progression(self) -> List[str]:
        ...
        for career_file in career_files:
            try:
                ...
                results.append(
                    f"Created career progression visualizations for {person_name}"
                )

            except Exception as e:
                results.append(
                    f"Error processing career data for {career_file.stem}: {str(e)}"
                )
    ...
    def _create_career_visualizations(
        ...
        try:
            ...
            timeline_file = (
                output_dir / f"career_timeline_{datetime.now().strftime('%Y%m%d')}.md"
            )

            # Use the new _save_mermaid_file method for validation and saving
            if hasattr(self, "_save_mermaid_file"):
                self._save_mermaid_file(timeline, timeline_file, person_name)
            else:
                # Fallback to direct write if the method doesn't exist (backward compatibility)
            ...
            skills_file = (
                output_dir / f"skills_radar_{datetime.now().strftime('%Y%m%d')}.json"
            )
        ...
        for evento in eventos:
            ...
            if date_str:
                try:
                    ...
                    year_num = int(year)
                    if 1900 <= year_num <= 2100:  # Reasonable range check
                        if year not in years:
                            years[year] = []
                        years[year].append(evento)
                except (ValueError, IndexError):
                    # If date parsing fails, put in "Unknown" section
                    if "Unknown" not in years:
                        years["Unknown"] = []
                    years["Unknown"].append(evento)

        # Add events to timeline by year
        for year in sorted(years.keys()):
            # Ensure section has proper format
            ...
            for evento in years[year]:
                ...
                # Escape any special characters in the details
                detalhes = (
                    evento.get("detalhes", "")
                    .replace(":", "&#58;")
                    .replace(",", "&#44;")
                ...
                # Ensure proper indentation and escape characters that could break MermaidJS
                mermaid += f"        {event_text.replace('\n', ' ')}\n"

        mermaid += "```\n"

        return mermaid

    def _create_skills_radar(
        self, person_name: str, career_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for skill_name, level in skills.items():
            ...
            categorized_skills[category].append({"name": name, "level": level})

        # Format for visualization
        radar_data = {"name": person_name, "categories": []}

        for category, skills_list in categorized_skills.items():
            radar_data["categories"].append({"name": category, "skills": skills_list})
            ...
            if metricas:
                ...
                if metricas.get("potencial_lideranca", 0) >= 7:
                    recommendations.append(
                        "**Desenvolver liderança formal**: Oferecer oportunidades de liderar projetos ou equipes pequenas para desenvolver habilidades de liderança."
                    )
                elif metricas.get("potencial_lideranca", 0) >= 4:
                    recommendations.append(
                        "**Desenvolver liderança técnica**: Incentivar mentoria de colegas menos experientes e liderança em áreas de especialidade."
                    )
                else:
                    recommendations.append(
                        "**Fortalecer fundamentos**: Focar em desenvolver domínio técnico e habilidades de comunicação antes de papéis de liderança."
                    )

                # Skill development
                if metricas.get("diversidade_habilidades", 0) <= 2:
                    recommendations.append(
                        "**Expandir repertório de habilidades**: Identificar áreas complementares para desenvolvimento que aumentem versatilidade na equipe."
                    )

                # Collaboration development
                if metricas.get("crescimento_colaborativo", 0) <= 5:
                    recommendations.append(
                        "**Aumentar colaboração**: Incentivar participação em comunidades de prática, projetos cross-funcionais e compartilhamento de conhecimento."
                    )

                # Team role alignment
                if avg_skill >= 4 and leadership >= 7:
                    recommendations.append(
                        "**Papel potencial na equipe**: Tech Lead ou Especialista Técnico Senior - pessoa pode servir como referência técnica e orientar direções tecnológicas."
                    )
                elif avg_skill >= 3.5 and collab >= 7:
                    recommendations.append(
                        "**Papel potencial na equipe**: Integrador ou Facilitador - pessoa pode ajudar a conectar diferentes áreas e promover colaboração."
                    )
                elif avg_skill >= 4 and collab <= 5:
                    recommendations.append(
                        "**Papel potencial na equipe**: Especialista Técnico - pessoa pode se aprofundar em áreas específicas e servir como referência."
                    )
                elif metricas.get("taxa_aquisicao_habilidades", 0) >= 3:
                    recommendations.append(
                        "**Papel potencial na equipe**: Inovador ou Early Adopter - pessoa pode liderar adoção de novas tecnologias e abordagens."
                    )
...
"""
            ...
            if certificacoes:
                ...
                for cert in sorted(
                    certificacoes,
                    key=lambda x: x.get("date_obtained", ""),
                    reverse=True,
                ):
                    nome = cert.get("name", "")
                    ...
                    data_expiracao = (
                        cert.get("expiry_date", "").replace("-", "/")
                        if cert.get("expiry_date")
                        else "N/A"
                    )

                    report += (
                        f"| {nome} | {emissor} | {data_obtencao} | {data_expiracao} |\n"
                    )
            else:
                report += "| - | - | - | - |\n"
...
"""
            ...
            if mentorias:
                ...
                for mentor in sorted(
                    mentorias, key=lambda x: x.get("start_date", ""), reverse=True
                ):
                    nome_mentor = mentor.get("mentor_name", "")
                    data_inicio = mentor.get("start_date", "").replace("-", "/")
                    data_fim = (
                        mentor.get("end_date", "").replace("-", "/")
                        if mentor.get("end_date")
                        else "Ativo"
                    )
...
"""
    ...
    def _create_comprehensive_report(
        ...
        manager_report: Path,
        career_report: Optional[Path] = None,
    ) -> str:
        ...
        try:
            ...
            indiv_strengths = self._extract_section(
                individual_content, "Analysis", end_marker="Generated on"
            )
            manager_strengths = self._extract_section(
                manager_content, "Pontos Fortes", end_marker="Áreas de Desenvolvimento"
            )
            manager_dev_areas = self._extract_section(
                manager_content, "Áreas de Desenvolvimento", end_marker="Áreas de Foco"
            )
...
"""
            ...
            combined_rating = self._calculate_combined_rating(
                indiv_score, manager_score
            )
...
"""
            ...
            if career_content:
                promotion_velocity = self._extract_context(
                    career_content, "Velocidade média de promoção"
                )
                avg_skill_level = self._extract_context(
                    career_content, "Nível médio de habilidades"
                )
...
"""
    ...
    def _calculate_combined_rating(
        self, indiv_score: Optional[str], manager_score: Optional[str]
    ) -> str:
    ...
    def _extract_section(
        self, content: str, section_name: str, end_marker: str = None
    ) -> str:
    ...
    def _extract_context(
        ...
        start_idx = max(
            0, content.rfind(".", max(0, keyword_idx - context_size), keyword_idx) + 1
        )
    ...
    def _create_pdi(
        ...
        try:
            ...
            dev_areas = self._extract_section(
                manager_content, "Áreas de Desenvolvimento", end_marker="Áreas de Foco"
            )
            ...
            career_goals = None
            if career_content:
                career_goals = self._extract_section(
                    career_content,
                    "## Metas de Carreira",
                    end_marker="## Certificações",
                )

            hoje = datetime.now().strftime("%d/%m/%Y")
            proximo_trimestre = (datetime.now() + timedelta(days=90)).strftime(
                "%d/%m/%Y"
            )
...
"""
        ...
        for template_file in template_files:
            try:
                ...
                if "eventos_carreira" in data or "matriz_habilidades" in data:
                    ...
                    results.append(
                        f"Skipped {template_file.name}: Unknown template format"
                    )

            except Exception as e:
                results.append(
                    f"Error processing template {template_file.name}: {str(e)}"
                )
    ...
    def _save_mermaid_file(
        self, mermaid_content: str, output_file: Path, context: str = ""
    ) -> None:
        """
        Validate and save a MermaidJS diagram to file

        Args:
            mermaid_content: The MermaidJS diagram content
            output_file: The file path to save to
            context: Context information for error messages (e.g., person name)
        """
        try:
            # Validate the MermaidJS content
            clean_content = self._validate_mermaid(mermaid_content, context)
            ...
            if self.verbose:
                print(
                    f"Successfully saved validated MermaidJS diagram to {output_file}"
                )

        except Exception as e:
            error_msg = f"Error validating/saving MermaidJS for {context}: {str(e)}"
            if self.ignore_errors:
                print(f"WARNING: {error_msg}")
            else:
                raise Exception(error_msg)

    def _validate_mermaid(self, mermaid_content: str, context: str = "") -> str:
        """
        Validate and fix MermaidJS diagram syntax
        ...
        Returns:
            Validated MermaidJS content
        """
        # Check if the content has the proper ```mermaid wrapper
        if not mermaid_content.strip().startswith("```mermaid"):
            mermaid_content = "```mermaid\n" + mermaid_content

        if not mermaid_content.strip().endswith("```"):
            mermaid_content = mermaid_content + "\n```"

        # Common MermaidJS syntax errors to check and fix
        ...
        for i, line in enumerate(lines):
            ...
            if "```" in line and in_mermaid_block:
                in_mermaid_block = False
                fixed_lines.append(line)
                continue
            ...
            # Fix common syntax errors within mermaid blocks
            fixed_line = line

            # Fix: Ensure timeline notation has proper spacing
            if "timeline" in fixed_line and not fixed_line.strip() == "timeline":
                fixed_line = "timeline"

            # Fix: Ensure section headers have proper format
            if fixed_line.strip().startswith("section") and ":" not in fixed_line:
                fixed_line = fixed_line + ":"

            # Fix: Ensure arrows have proper syntax in flowcharts and sequence diagrams
            if "-->" in fixed_line and " --> " not in fixed_line:
                fixed_line = fixed_line.replace("-->", " --> ")

            # Fix: Ensure proper indentation in nested elements
            if fixed_line.strip() and not fixed_line.strip().startswith(
