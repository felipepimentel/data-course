...
class DataPipeline:
    ...
    def process_career_data(self, person_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Calculate metrics
            metricas = self._calculate_career_metrics(eventos, habilidades, certificacoes, metas, mentoria)
    ...
    def _calculate_career_metrics(self, 
                                 eventos: List[Dict[str, Any]], 
                                 habilidades: Dict[str, int],
                                 certificacoes: List[Dict[str, Any]],
                                 metas: List[Dict[str, Any]],
                                 mentoria: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if promocoes:
            ...
            if ultima_promocao:
                try:
                    data_promocao = datetime.strptime(ultima_promocao, "%Y-%m-%d").date()
                    metricas["dias_ultima_promocao"] = (datetime.now().date() - data_promocao).days
                    metricas["meses_ultima_promocao"] = metricas["dias_ultima_promocao"] / 30.44  # Average days per month
                except:
                    pass
        ...
        if len(promocoes) >= 2:
            try:
                ...
                for i in range(1, len(promocoes)):
                    data_anterior = datetime.strptime(promocoes[i-1].get("data", ""), "%Y-%m-%d").date()
                    data_atual = datetime.strptime(promocoes[i].get("data", ""), "%Y-%m-%d").date()
                    dias = (data_atual - data_anterior).days
                    intervalos.append(dias)
                
                # Calculate average
                if intervalos:
                    metricas["media_dias_entre_promocoes"] = sum(intervalos) / len(intervalos)
                    metricas["media_anos_entre_promocoes"] = metricas["media_dias_entre_promocoes"] / 365.25
            except:
                pass
        ...
        if habilidades:
            ...
            skill_events = [e for e in eventos if e.get("tipo_evento") == "skill_acquisition"]
            if len(skill_events) >= 2:
                try:
                    # Get earliest and latest skill events
                    skill_events.sort(key=lambda x: x.get("data", ""))
                    primeiro = datetime.strptime(skill_events[0].get("data", ""), "%Y-%m-%d").date()
                    ultimo = datetime.strptime(skill_events[-1].get("data", ""), "%Y-%m-%d").date()
                    dias_total = (ultimo - primeiro).days
                    
                    if dias_total > 0:
                        metricas["taxa_aquisicao_habilidades_anual"] = (len(skill_events) / dias_total) * 365.25
                except:
                    pass
        ...
        if certificacoes:
            ...
            if len(certificacoes) >= 2:
                try:
                    ...
                    sorted_certs = sorted(certificacoes, key=lambda x: x.get("date_obtained", ""))
                    primeiro = datetime.strptime(sorted_certs[0].get("date_obtained", ""), "%Y-%m-%d").date()
                    ultimo = datetime.strptime(sorted_certs[-1].get("date_obtained", ""), "%Y-%m-%d").date()
                    dias_total = (ultimo - primeiro).days
                    
                    if dias_total > 0:
                        metricas["taxa_aquisicao_certificacoes_anual"] = (len(certificacoes) / dias_total) * 365.25
                except:
                    pass
        ...
        if metas:
            ...
            metricas["metas_concluidas"] = len([m for m in metas if m.get("status") == "completed"])
            metricas["metas_em_andamento"] = len([m for m in metas if m.get("status") == "in_progress"])
            metricas["metas_nao_iniciadas"] = len([m for m in metas if m.get("status") == "not_started"])
            metricas["metas_atrasadas"] = len([m for m in metas if m.get("status") == "delayed"])
            
            if metricas["total_metas"] > 0:
                metricas["taxa_conclusao_metas"] = metricas["metas_concluidas"] / metricas["total_metas"] * 100
        
        # Process mentorship metrics
        if mentoria:
            metricas["total_mentorias"] = len(mentoria)
            metricas["mentorias_ativas"] = len([m for m in mentoria if m.get("active", False)])
            
            # Calculate average mentorship duration
            duracoes = []
            for mentor in mentoria:
                try:
                    inicio = datetime.strptime(mentor.get("start_date", ""), "%Y-%m-%d").date()
                    ...
                    if mentor.get("active", False):
                        ...
                        # Use end date
                        fim = datetime.strptime(mentor.get("end_date", ""), "%Y-%m-%d").date()
            ...
            if duracoes:
                ...
                metricas["media_meses_mentoria"] = metricas["media_dias_mentoria"] / 30.44
        ...
        if "media_habilidades" in metricas:
            growth_score += metricas["media_habilidades"] * 5  # 1-5 scale becomes 5-25 points
        ...
        if len(habilidades) > 0:
            ...
            for skill_name in habilidades:
                categoria = skill_name.split('.')[0] if '.' in skill_name else 'technical'
                if categoria not in categorias:
                    categorias[categoria] = 0
                categorias[categoria] += 1
            
            # More categories with at least 2 skills is better
            categorias_relevantes = len([c for c, count in categorias.items() if count >= 2])
        ...
        if "taxa_aquisicao_habilidades" in metricas and metricas["taxa_aquisicao_habilidades"] > 0:
        ...
        leadership_skills = [s for s in habilidades.keys() if 'lideranca' in s.lower() or 'leadership' in s.lower()]
        if leadership_skills:
            avg_leadership = sum(habilidades[s] for s in leadership_skills) / len(leadership_skills)
        ...
        collaborative_events = [e for e in eventos if 
                               'compartilhamento' in e.get('detalhes', '').lower() or
                               'apresentacao' in e.get('detalhes', '').lower() or
                               'sharing' in e.get('detalhes', '').lower() or
                               'presentation' in e.get('detalhes', '').lower() or
                               'workshop' in e.get('detalhes', '').lower()]
        
        collaborative_score += min(5, len(collaborative_events) * 1.25)
        
        # Check for cross-functional skills/experiences
        cross_functional = len(set([e.get('tipo_evento') for e in eventos if e.get('tipo_evento') in ['lateral_move', 'role_change']]))
    ...
    def generate_team_development_report(self) -> Optional[Path]:
        ...
        try:
            ...
            report_path = team_dev_dir / f"team_development_{current_time.strftime('%Y%m%d_%H%M%S')}.md"
    ...
    def _generate_team_development_content(self, career_data: Dict[str, Dict[str, Any]]) -> str:
...
"""
        ...
        if avg_skill_levels:
            weak_skills = [category for category, avg in avg_skill_levels.items() if avg < 3.0]
            
            if weak_skills:
                content += "As seguintes áreas de habilidades precisam de desenvolvimento:\n\n"
                for skill in weak_skills:
                    content += f"- **{skill}**: Nível médio {avg_skill_levels[skill]:.1f}/5\n"
            else:
...
"""
        ...
        if not (tech_leads or specialists or integrators or innovators):
            content += "Dados insuficientes para identificar papéis potenciais na equipe.\n"
        ...
        if has_tech_leadership and has_specialists and has_integrators and has_innovators:
...
"""
        ...
        # Check team balance
        if not has_tech_leadership:
            recommendations.append("**Desenvolver Liderança Técnica**: Identificar membros com potencial de liderança e oferecer oportunidades de desenvolvimento através de mentoria, cursos especializados e responsabilidades incrementais em projetos.")
        
        if not has_integrators:
            recommendations.append("**Fomentar Colaboração Cross-Funcional**: Implementar projetos que exijam colaboração entre diferentes especialidades e reconhecer explicitamente contribuições colaborativas.")
        
        if not has_innovators:
            recommendations.append("**Estimular Cultura de Aprendizado**: Estabelecer comunidades de prática, clubes de estudo ou tempo dedicado para exploração de novas tecnologias.")
        ...
        high_performer_ratio = len(high_performers) / len(career_data) if career_data else 0
        if high_performer_ratio < 0.2:  # Less than 20% high performers
            recommendations.append("**Elevação de Performance**: Implementar programas específicos para identificar e desenvolver potenciais high performers, como programas de aceleração e mentoria com líderes experientes.")
