...
class Visualization:
        ...
        try:
            ...
            if "filtered_data" in data:
                ...
                for item in data["filtered_data"]:
                    ...
                    behavior = item.get(
                        "Comportamento", item.get("Behavior", "Unknown")
                    )
    ...
    def save_or_show(
        self, fig: plt.Figure, filename: Optional[str] = None
    ) -> Optional[str]:
    ...
    def scatter_plot(
        ...
        # Create scatter plot
        sns.scatterplot(
            data=data, x=x, y=y, hue=hue, size=size, palette=config.palette, ax=ax
        )
    ...
    def multi_chart_grid(
        ...
        for i, chart_config in enumerate(charts):
            ...
            if chart_type == "bar":
                ...
                if isinstance(data, pd.DataFrame):
                    ax.scatter(
                        data[chart_config.get("x", data.columns[0])],
    ...
    def create_career_progression_charts(
        self, person_name: str, career_data: Dict[str, Any], output_path: Path
    ) -> None:
    ...
    def _create_career_timeline(
        self, person_name: str, events: List[Dict[str, Any]], output_path: Path
    ) -> None:
...
"""
            ...
            for event in sorted_events:
                ...
                details = event.get("detalhes", "").replace(
                    "'", "\\'"
                ...
                if new_position and (
                    event_type in ["promotion", "lateral_move", "role_change"]
                ):
...
"""
    ...
    def _create_mermaid_timeline(
        self, person_name: str, events: List[Dict[str, Any]]
    ) -> str:
...
"""
    ...
    def _create_skills_radar(
        self, person_name: str, skills: Dict[str, int], output_path: Path
    ) -> None:
...
"""
    ...
    def _create_growth_metrics_chart(
        self, person_name: str, metrics: Dict[str, Any], output_path: Path
    ) -> None:
...
"""
            ...
            self.logger.error(
                f"Error creating growth metrics chart for {person_name}: {e}"
            )

    def _create_career_dashboard(
        self, person_name: str, career_data: Dict[str, Any], output_path: Path
    ) -> None:
...
class DataVisualizer(Visualization):
    """
    Enhanced data visualization class that extends base Visualization with
    additional specialized visualization capabilities for People Analytics data.
    """

    def __init__(self, custom_style: Optional[str] = None):
        """
        Initialize the DataVisualizer with optional custom style.

        Args:
            custom_style: Optional custom style for visualizations (default: None)
        """
        super().__init__()
    ...
    def create_performance_overview(
        ...
        """
        Create a performance overview visualization with multiple charts.
        ...
        Returns:
            Path to saved visualization or None if shown interactively
        """
        if data.empty:
            logger.warning("No data provided for performance overview")
            return None

        # Create a multi-chart grid with 2x2 layout
        charts = [
            {
                "type": "bar",
                "data": data,
            ...
            },
            {
                ...
                "data": data.pivot_table(
                    index="Person", columns="Year", values="Score", aggfunc="mean"
                ),
                "title": "Performance Heatmap",
            },
        ]

        return self.multi_chart_grid(
            charts,
            rows=2,
            cols=2,
            suptitle="Performance Overview",
            figsize=(14, 10),
            filename=output_path,
        )

    def create_nine_box_matrix(
        self,
        ...
        """
        Create a Nine-Box matrix visualization.

        Args:
            performance_data: DataFrame with performance data
            potential_data: DataFrame with potential data
            output_path: Optional path to save the visualization
        ...
        if merged_data.empty:
            logger.warning("No matching data for Nine-Box matrix")
            return None

        # Create config
        config = ChartConfig(
            title="Nine-Box Talent Matrix",
            xlabel="Performance",
            ylabel="Potential",
            figsize=(12, 10),
        )
        ...
        # Plot points
        scatter = ax.scatter(
            merged_data["Score_perf"],
            merged_data["Score_pot"],
            s=100,
            alpha=0.7,
            c=merged_data["Year"].astype("category").cat.codes,
            cmap="viridis",
        )
        ...
        for i, row in merged_data.iterrows():
            ax.annotate(
                row["Person"],
                (row["Score_perf"], row["Score_pot"]),
                xytext=(5, 5),
                textcoords="offset points",
            )

        # Add grid lines for the 9-box
        ax.axhline(y=3.33, color="gray", linestyle="--", alpha=0.5)
        ax.axhline(y=6.66, color="gray", linestyle="--", alpha=0.5)
        ax.axvline(x=3.33, color="gray", linestyle="--", alpha=0.5)
        ax.axvline(x=6.66, color="gray", linestyle="--", alpha=0.5)
        ...
        legend = ax.legend(
            ...
            labels=sorted(merged_data["Year"].unique()),
            title="Year",
            loc="best",
        )
        ...
        # Add box labels
        box_labels = [
            (1.7, 8.3, "Low Performer\nHigh Potential"),
            (5.0, 8.3, "Core Player\nHigh Potential"),
            (8.3, 8.3, "Star\nHigh Potential"),
            (1.7, 5.0, "Low Performer\nMedium Potential"),
            (5.0, 5.0, "Core Player\nMedium Potential"),
            (8.3, 5.0, "Star\nMedium Potential"),
            (1.7, 1.7, "Low Performer\nLow Potential"),
            (5.0, 1.7, "Core Player\nLow Potential"),
            (8.3, 1.7, "Star\nLow Potential"),
        ]

        for x, y, label in box_labels:
            ax.text(
                x,
                y,
                label,
        ...
        return self.save_or_show(fig, output_path)
